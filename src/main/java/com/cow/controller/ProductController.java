...
import com.cow.service.*;
import com.cow.util.general.CommonResult;
import com.cow.util.recommend.RecommendationService;
...
public class ProductController {
    ...
    @ResponseBody
    public CommonResult recommendItemsForUser(@RequestParam(name = "topN", defaultValue = "10") int topN) {

        // 获取用户和商品的交互列表
        Map<String, Map<String, Double>> userItemMatrix = productReviewService.getUserItemMatrix();
        ...
        // 如果用户未登录，则随机返回5条商品
        if (user == null) {
            List<Product> products = productService.selectAll().subList(0, 5);
            return CommonResult.success("推荐成功", products);
        }

        List<String> productNos = recommendationService.recommendItemsForUser(user.getAccountNumber(), topN, userItemMatrix);
        // 如果推荐列表为空，则随机返回5条商品
        ...
        // 如果推荐列表productNos小于5条，则从商品列表中补全剩余商品
        if (productNos.size() < 5) {
            Integer count = 5 - productNos.size();
            List<Product> products = productService.selectAll();
            // 排除推荐列表中已有的商品，防止商品重复推荐
            int i = 0;
            while (i < count){
                Integer index = (int) (Math.random() * products.size());
            ...
            productNos = productNos.subList(0, 5);
        }

        // 根据商品编号列表查询推荐的商品详情信息
        List<Product> products = productService.selectByNos(productNos);
